...
localización. Los códigos XML/CFDI de esta tabla están activos sólo para la
vertical XML individual de Fase 1. Los códigos de capacidades posteriores
siguen reservados y no autorizan su implementación.

El boundary HTTP de Fase 1 devuelve el envelope estándar de Balanz con `code`,
...
| `MFA_SETUP_REQUIRED`               | F0/F1 activo                                  |             403 | no                           | info         | sí                        | Configura la verificación en dos pasos para continuar.       | Completar enrolamiento MFA.                                            |
| `MFA_REQUIRED`                     | F0/F1 activo                                  |             401 | no                           | info         | sí                        | Confirma tu identidad para continuar.                        | Completar MFA en la sesión.                                            |
| `IDEMPOTENCY_KEY_REQUIRED`         | F1 activo                                     |             400 | no                           | info         | no                        | Falta la clave de idempotencia.                              | Reenviar con una key nueva y opaca.                                    |
...
| `INGESTION_ACTIVE_JOB_LIMIT`       | F1 activo                                     |             429 | no                           | info         | sí                        | Se alcanzó el límite de procesos activos.                     | Esperar a que termine un proceso y reintentar la misma operación.      |
...
| `INGESTION_FILE_REQUIRED`          | F1 activo                                     |             400 | no                           | info         | no                        | Selecciona un archivo para continuar.                        | Adjuntar exactamente el archivo permitido.                             |
| `INGESTION_UNSUPPORTED_MEDIA_TYPE` | F1 activo                                     |             415 | no                           | info         | sí                        | El tipo de archivo no es compatible.                         | Usar XML individual en `multipart/form-data`.                          |
| `INGESTION_FILE_TOO_LARGE`         | F1 activo                                     |             413 | no                           | warn         | sí                        | El archivo supera el límite permitido.                       | Mantener el XML individual dentro de 5 MiB.                            |
| `INGESTION_TOO_MANY_FILES`         | F1 activo; F2 reutilizable                    |             400 | no                           | warn         | sí                        | La solicitud contiene más archivos de los permitidos.        | Enviar exactamente un XML individual.                                 |
| `INGESTION_UPLOAD_ABORTED`         | F1 activo                                     |             408 | no                           | info         | sí                        | La transferencia fue cancelada antes de completarse.         | Recuperar con la misma key hasta confirmar el estado durable.          |
| `INGESTION_UPLOAD_FAILED`          | F1 activo                                     |             409 | no                           | info         | sí                        | La recepción anterior falló definitivamente.                 | Renovar la key y volver a transferir el mismo archivo.                 |
...
| `XML_MALFORMED`                    | F1 activo                                     | resultado item | no                           | info         | sí                        | El XML no tiene una estructura válida.                       | Corregir el documento de origen.                                       |
| `XML_SECURITY_VIOLATION`           | F1 activo                                     | resultado item | no                           | warn         | sí                        | El XML fue rechazado por seguridad.                          | No reintentar el mismo contenido; revisar origen.                      |
| `CFDI_VERSION_UNSUPPORTED`         | F1 activo                                     | resultado item | no                           | info         | sí                        | La versión del CFDI no es compatible.                        | Usar CFDI 4.0.                                                         |
| `COMPLEMENT_UNSUPPORTED`           | F1 activo                                     | resultado/incidente | no                       | info         | sí                        | El complemento no está soportado.                            | Una versión inválida rechaza; un namespace desconocido incorpora core y abre incidente. |
| `CFDI_UUID_INVALID`                | F1 activo                                     | resultado item | no                           | warn         | sí                        | El comprobante no tiene un identificador fiscal válido.      | Verificar el original.                                                 |
| `CFDI_RFC_FOREIGN`                 | F1 activo                                     | resultado item | no                           | info         | sí                        | El comprobante no corresponde a la entidad seleccionada.     | Seleccionar la entidad correcta o revisar el XML.                      |
| `CFDI_DUPLICATE`                   | F1 activo                                     | resultado item | no                           | info         | sí                        | El comprobante ya fue incorporado.                           | Consultar el resultado existente.                                      |
| `CFDI_UUID_HASH_CONFLICT`          | F1 activo                                     | resultado item | no                           | error        | sí/alerta                 | El comprobante entra en conflicto con un original existente. | No reemplazar el original; investigar el incidente alto.               |
| `FISCAL_PERIOD_NOT_CONFIGURED`     | F1 activo                                     | incidente      | no                           | info         | sí                        | No existe un período fiscal compatible.                      | Configurar el ejercicio; el CFDI permanece incorporado.                |
...
| `JOB_NOT_RETRYABLE`                | F1 activo                                     |             409 | no                           | info         | sí                        | El trabajo no admite otro intento.                           | Revisar estado/error terminal.                                         |
...
| `RLS_CONTEXT_REQUIRED`             | F0/F1 activo interno                          |         500/403 | no                           | error        | sí/alerta                 | No se pudo establecer el contexto de acceso.                 | Fallar cerrado e investigar boundary transaccional.                    |
| `RLS_CONTEXT_INVALID`              | F0/F1 activo interno                          |         500/403 | no                           | error        | sí/alerta                 | El contexto de acceso no es válido.                          | Fallar cerrado; revisar resolución server-side.                        |
...
| `PARSER_INTERNAL_ERROR`            | F1 activo                                     | resultado job  | sí acotado                   | error        | sí                        | No se pudo procesar el documento.                            | Reintento durable; alertar al agotar intentos.                         |
...
`MALWARE_DETECTED`, `XML_MALFORMED`, `XML_SECURITY_VIOLATION`, versión raíz o
versión de complemento soportado no admitida, RFC ajeno, hash conflict y
límites de archivo no se resuelven reintentando los mismos bytes. Un namespace
de complemento desconocido no es rechazo: incorpora el core y abre incidente.
El job/item conserva procedencia y un resultado terminal seguro.
...
Cierre PR25: SAT_PAYROLL_UNSUPPORTED registra por entrada la nómina recibida inesperadamente desde SAT; no incorpora CFDI ni datos de nómina. SAT_PILOT_XML_ONLY impide metadata en el piloto real. EFIRMA_REAL_PILOT_NOT_AUTHORIZED deniega configuración operativa ausente, vencida, revocada o incompatible con generación/bundle/scope; errores sanitizados.
