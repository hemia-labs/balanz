...
## Cierre PR25 / piloto XML
El permiso operativo externo dura como máximo24h y limita una organización/entidad; su vencimiento o retiro impide nuevas operaciones sensibles. Es independiente del grant TOTP y de la custodia absoluta de diez minutos; ninguna consulta renueva ambos. Registros/folios/paquetes siguen siendo durables. Al entrar en espera SAT la operación de custodia termina inmediatamente: la siguiente consulta SAT requiere otra autorización del usuario; no se promete polling remoto autónomo.

Restauración: con API/worker consumidores detenidos, retirar el archivo EFIRMA_REAL_PILOT_AUTHORIZATION_FILE previo, rotar EFIRMA_CUSTODY_GENERATION_FILE fuera del snapshot y reconciliar. Un responsable debe emitir otra autorización operativa que vincule la nueva generación y el hash actual del bundle antes de habilitar. Restaurar el archivo viejo no supera la comprobación contra la generación externa. Las copias cifradas de backups/versionado conservan sus límites documentados; no hay afirmación de borrado físico total.

Integración focalizada del cierre: requiere reactivar el motor Docker Linux y los servicios QA aislados existentes; ejecución 2026-09-15 bloqueada por pipe dockerDesktopLinuxEngine inexistente. No reconstruir navegador ni servicios compartidos. Ejecutar test/external/sat.external.ts para los nuevos escenarios de más de20 procesos, dos paquetes con fallo temporal y actor asignado distinto; no sustituirlo por un PASS de mocks.
