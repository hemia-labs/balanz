...
- `WORKER_MAX_ATTEMPTS=3`: máximo tres ejecuciones totales, incluida la inicial;
  backoff 10 y 30 s más jitter antes de las ejecuciones 2 y 3. El valor 120 queda
  reservado y nunca habilita un cuarto claim.
...
- Completar exige worker/lease/version vigentes.
...
`balanz_worker`. Después de aplicar 060/061, aprovisiona los LOGINs dedicados
...
`INHERIT=false`, `SET=true`, y sin `BYPASSRLS`, owner, ACL fiscal directa,
...
`search_path=public`, funciones definer con resolución fija y bloquea ACLs
directas de tabla, columna, secuencia y función fiscal.
...
4. mantiene heartbeat sólo durante la ventana segura; si heartbeat devuelve
   ...
   worker/lease/version vigentes;
5. completa únicamente resultados con lease vigente o los abandona;
...
que otro worker/reconciliador recupere. No edites `locked_by` manualmente.
...
1. Identifica worker/lease/version y timestamps técnicos.
...
2. Validar config productiva sin mostrar secretos.
3. Ejecutar `npm run release:prepare` como init/release job con la identidad de
   migración, cerrar ese job y retirar su credencial del runtime.
4. Desplegar código compatible; iniciar workers con concurrencia controlada.
5. Verificar probes, RLS, claim, Redis fallback y adapters.
6. Observar queue age, leases, errors y reconciliación.
