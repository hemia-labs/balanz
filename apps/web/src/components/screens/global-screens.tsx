...
import { DefinitionGrid, FeaturePendingNotice, Field, ProgressValue, SectionTabs, Surface, SurfaceHeader } from "@/components/product-patterns";
...
export function OrganizationHomeScreen({ organizationId }: { organizationId: string }) {
  ...
  return <div className="space-y-6">
    ...
    <Surface>
      ...
      <LegacyFilterBar><Field label="Estado"><select className={selectClass}><option>Todos los estados</option><option>En revisión</option><option>Con novedades</option></select></Field><Field label="Responsable"><select className={selectClass}><option>Todos</option><option>Mariana Torres</option><option>Ángel Ruiz</option></select></Field><Button variant="outline">Limpiar filtros</Button></LegacyFilterBar>
      <DataTable caption="Estado operativo de clientes" rows={clients} rowKey={(row) => row.id} columns={[
        { id: "client", header: "Cliente", render: (row) => <div><Link className="font-semibold text-primary hover:underline" href={`${base}/clients/${row.id}/overview`}>{row.name}</Link><p className="identifier text-caption text-muted-foreground">{row.rfc}</p></div> },
        ...
        { id: "action", header: "Acción", render: (row) => <div className="flex gap-1"><Button render={<Link href={`${base}/clients/${row.id}/fiscal-years/2026/periods/08/overview`} />} variant="outline" size="sm">Abrir período</Button><DetailDrawer trigger={<Button variant="ghost" size="icon-sm" aria-label={`Vista rápida de ${row.name}`}><Eye /></Button>} title={row.name} description={row.rfc}><DefinitionGrid items={[{ label: "Estado", value: <StatusBadge status={row.status} /> }, { label: "Avance", value: `${row.progress}%` }, { label: "Incidencias", value: row.incidents }, { label: "e.firma", value: <StatusBadge status={row.eSignature} /> }]} /><Button render={<Link href={`${base}/clients/${row.id}/overview`} />} className="mt-5">Abrir cliente</Button></DetailDrawer></div> },
      ]} />
    </Surface>
    <div className="grid gap-6 xl:grid-cols-2">
      <Surface><SurfaceHeader title="Continuar trabajando" description="Contextos visitados recientemente." /><div className="divide-y divide-border">{clients.slice(0, 2).map((client) => <Link key={client.id} href={`${base}/clients/${client.id}/overview`} className="flex min-h-14 items-center justify-between px-5 py-3 hover:bg-muted/55"><span><span className="block font-semibold">{client.name}</span><span className="text-caption text-muted-foreground">{client.lastActivity}</span></span><StatusBadge status={client.status} /></Link>)}</div></Surface>
      <Surface><SurfaceHeader title="Procesos recientes" description="Trabajos autorizados del despacho." /><div className="divide-y divide-border">{demoData.processes.slice(0, 3).map((process) => <Link key={process.id} href={`${base}/processes`} className="flex min-h-14 items-center justify-between px-5 py-3 hover:bg-muted/55"><span><span className="block font-semibold">{process.type}</span><span className="text-caption text-muted-foreground">{clientById(organizationId, process.clientId)?.name}</span></span><StatusBadge status={process.status} /></Link>)}</div></Surface>
    </div>
  </div>;
}
...
export function AuditScreen() {
  ...
  return <div className="space-y-6"><header className="border-l-2 border-brand-mark pl-4"><p className="text-caption font-semibold text-accent-foreground">Trazabilidad</p><h1 className="text-heading-lg font-bold">Auditoría</h1><p className="mt-1 text-body text-muted-foreground">Consulta eventos operativos sin exponer secretos, credenciales o XML completo.</p></header><Surface><LegacyFilterBar><Field label="Fecha"><Input type="date" defaultValue="2026-08-18" /></Field><Field label="Actor"><Input placeholder="Nombre" /></Field><Field label="Módulo"><select className={selectClass}><option>Todos</option><option>Procesos</option><option>CFDI</option></select></Field></LegacyFilterBar><DataTable caption="Eventos de auditoría" rows={events} rowKey={(row) => row.id} columns={[
...
const settingsLabels: Record<string, { title: string; description: string }> = {
  overview: { title: "Configuración del despacho", description: "Administra datos, seguridad, licencia y soporte según tu capacidad." },
  data: { title: "Datos del despacho", description: "Identidad operativa y preferencias generales del tenant." },
  security: { title: "Seguridad", description: "Políticas y controles de acceso del despacho." },
  "billing-plan": { title: "Plan y facturación", description: "Propiedad contractual, plan y método de pago; sólo Titular." },
  "data-retention": { title: "Retención y exportación de datos", description: "Políticas de conservación y solicitudes autorizadas." },
  support: { title: "Soporte", description: "Acceso temporal y ayuda sin convertir soporte en un rol permanente." },
};

export function OrganizationSettingsScreen({ organizationId, section = "overview" }: { organizationId: string; section?: string }) {
  const base = `${organizationBase("es", organizationId)}/settings`;
  const copy = settingsLabels[section] ?? settingsLabels.overview;
  const links = [{ id: "data", label: "Datos", href: `${base}/data` }, { id: "security", label: "Seguridad", href: `${base}/security` }, { id: "billing-plan", label: "Plan y facturación", href: `${base}/billing-plan` }, { id: "data-retention", label: "Retención de datos", href: `${base}/data-retention` }, { id: "support", label: "Soporte", href: `${base}/support` }];
  return <div className="space-y-6"><header className="border-l-2 border-brand-mark pl-4"><p className="text-caption font-semibold text-accent-foreground">Administración del despacho</p><h1 className="text-heading-lg font-bold">{copy.title}</h1><p className="mt-1 text-body text-muted-foreground">{copy.description}</p></header><Surface><SectionTabs items={links} active={section} /><div className="space-y-5 p-5"><FeaturePendingNotice>Los controles están preparados para contratos futuros; ningún cambio se persiste.</FeaturePendingNotice>{section === "billing-plan" ? <DefinitionGrid items={[{ label: "Propietaria contractual", value: "Mariana Torres" }, { label: "Plan", value: "Sin contrato conectado" }, { label: "Método de pago", value: "No disponible" }, { label: "Renovación", value: "No disponible" }]} /> : <div className="grid max-w-form gap-5 sm:grid-cols-2"><Field label="Nombre del despacho"><Input defaultValue="Estudio Contable Norte" /></Field><Field label="Zona horaria"><select className={selectClass}><option>America/Mexico_City</option></select></Field><Field label="Correo operativo"><Input type="email" defaultValue="operacion@example.test" /></Field><Field label="Preferencia de período"><select className={selectClass}><option>Mes actual</option></select></Field></div>}<div className="flex justify-end"><Button disabled>Guardar cambios</Button></div></div></Surface></div>;
}
