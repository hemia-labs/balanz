...
const selectClass = "h-10 w-full rounded-md border border-input bg-card px-3 text-body-sm";
...
export function CfdiActions({
  ...
  return (
    <>
      <ActionDialog
        trigger={<Button><DownloadCloud />Descargar del SAT</Button>}
        title="Iniciar descarga SAT"
        description={`${clientName} · ${periodLabel}`}
        confirmLabel="Solicitar descarga"
      >
        <FeaturePendingNotice>No existe integración SAT. La solicitud no se enviará.</FeaturePendingNotice>
        <Field label="Período"><Input value={periodLabel} readOnly /></Field>
      ...
      <ActionDialog
        trigger={<Button variant="outline"><Upload />Cargar XML o ZIP</Button>}
        title="Cargar XML o ZIP"
        description={`${clientName} · ${periodLabel}`}
        confirmLabel="Cargar archivos"
      >
        <FeaturePendingNotice>No se transferirán archivos. El servicio de cargas está pendiente.</FeaturePendingNotice>
        <Field label="Archivos"><Input type="file" accept=".xml,.zip" multiple /></Field>
      </ActionDialog>

      {includeExport ? (
        <ActionDialog
          trigger={<Button variant="outline"><FileArchive />Exportar</Button>}
        ...
        >
          <FeaturePendingNotice>El servicio de exportación no está conectado.</FeaturePendingNotice>
          <Field label="Formato">
