  ...
  async function resend() {
    ...
    try {
      ...
      if (error instanceof ApiError && error.status === 429) {
        setMessage(
          "Ya solicitaste un enlace recientemente. Intenta de nuevo más tarde.",
        );
      } else {
        setMessage(
          "Si el correo puede verificarse, recibirás un nuevo enlace.",
        );
      }
    }
  }
      ...
      <Surface className="w-full max-w-form space-y-5 p-6">
        <div>
          ...
          <p className="mt-2 text-body-sm text-muted-foreground">
            ...
            {pending.email ? ` (${pending.email})` : ""}. Te enviamos un enlace
        ...
        {message ? (
          <p
            role="status"
            aria-live="polite"
            className="rounded-md border border-border bg-muted/30 p-3 text-body-sm"
          >
            {message}
          </p>
        ) : null}
        {state === "success" ? (
          <div className="space-y-4">
            <p className="text-body-sm text-success">
              Correo confirmado. Configura la verificación en dos pasos para
              ...
              <dl className="grid gap-2 rounded-md border border-border bg-muted/30 p-3 text-body-sm sm:grid-cols-2">
                <div>
                  ...
                  <dd className="font-semibold">
                    {result.emailVerified ? "Verificado" : "Pendiente"}
                  </dd>
                </div>
                <div>
                  <dt className="text-muted-foreground">Suscripción</dt>
                  <dd className="font-semibold">{result.subscriptionType}</dd>
                </div>
                <div>
                  <dt className="text-muted-foreground">Trial</dt>
                  <dd className="font-semibold">{result.trial.status}</dd>
                </div>
                <div>
                  <dt className="text-muted-foreground">Siguiente paso</dt>
                  <dd className="font-semibold">{result.nextStep}</dd>
                </div>
              </dl>
            ) : null}
            <Button
          ...
          <div className="space-y-3">
            {token ? (
              ...
              >
                {state === "confirming" ? "Confirmando…" : "Confirmar correo"}
              </Button>
            ) : null}
            {state === "error" ? (
              <p role="alert" className="text-body-sm text-destructive">
                {message ?? "El enlace es inválido, expiró o ya fue usado."}
              </p>
            ) : null}
            <Button
              type="button"
