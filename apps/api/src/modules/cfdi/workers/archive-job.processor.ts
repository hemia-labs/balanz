import { SAT_PACKAGE_LIMITS } from '../../sat-download/sat-contract';
...
@Injectable()
export class ArchiveJobProcessor implements IngestionJobHandler {
  readonly source: 'manual_zip' | 'sat_package' = 'manual_zip';
  constructor(
    ...
    @Optional() private readonly metadata?: SatMetadataProcessor,
  ) {}
  ...
  async handle(job: ClaimResult, signal: AbortSignal) {
    ...
    try {
      ...
      const extractor = new ZipExtractor(
        this.storage,
        job.sourceType === 'sat_package' ? SAT_PACKAGE_LIMITS : ZIP_LIMITS,
      );
      ...
      for (const entry of archive.entries)
        reservations.set(
          entry.ordinal,
          await this.persistence.reserve(
            job,
            root.contentType === 'metadata' ? { ...entry, xml: false } : entry,
          ),
        );
      const uncompressed = await extractor.extract(
        ...
        async (entry, stream) => {
          ...
          await this.persistence.extracted(
            job,
            item,
            entry,
            result,
            root.contentType === 'metadata' && entry.metadataText === true,
          );
        },
        () => this.persistence.boundary(job),
        root.contentType === 'metadata',
      );
      ...
      if (root.contentType === 'metadata') {
        ...
        await extractor.extract(
          ...
          signal,
          async (entry, stream) => {
            await this.metadata!.process(job, entry, stream, signal);
          },
          () => this.persistence.boundary(job),
        );
